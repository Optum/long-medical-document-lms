...
# And LM checkpoint after continued pretraining on data
# Or point to a model from the HuggingFace Model Hub
MODEL = "/mnt/azureblobshare/models/bigbird_imdb/run_9/checkpoint-1000/"
LM = "/mnt/azureblobshare/models/bigbird_for_mlm_imdb/run_9/checkpoint-920/"
...
# Output path
OUTPUT_PATH = f"./soc_results_{DATA}/"  # will be deleted if it already exists
...
lm = AutoModelForMaskedLM.from_pretrained(LM)
...
for s, (doc_input_ids, doc_y) in enumerate(
    ...
    print(f"Running SOC for sample {s} of {NUM_SAMPLE}...")
...
print(f"Ran a total of {total_num_inferences} model inferences.")
